...
namespace CrudOperations.DataAccessLayer
{
    public class CrudOperationDL : ICrudOperationDL
    {
        ...
        public CrudOperationDL(IConfiguration configuration)
        {
            ...
            _mongoConnection = new MongoClient(_configuration["CrudDatabase:ConnectionString"]);
            var MongoDataBase = _mongoConnection.GetDatabase(_configuration["CrudDatabase:DatabaseName"]);
            _userCollection = MongoDataBase.GetCollection<InsertRecordRequest>(_configuration["CrudDatabase:UserCollectionName"]);
        }
        ...
        public async Task<GetRecordByNameResponse> GetRecordByName(string Name)
        {
            ...
            {
                response.data = new List<InsertRecordRequest>();
                response.data = await _userCollection.Find(x => (x.FirstName == Name) || (x.LastName == Name)).ToListAsync();
                if (response.data.Count==0)
                {
                    response.Message = "No Data Found";
                }
        ...
        public async Task<UpdateRecordByIdResponse> UpdateSalaryById(UpdateSalaryByIdRequest request)
        {
            ...
            try
            {
                ...
                var Result = await _userCollection.UpdateOneAsync(x=>x.Id==request.Id, updateDoc);
        ...
        public async Task<DeleteRecordByIdResponse> DeleteRecordById(DeleteRecordByIdRequest request)
        {
            ...
            try
            {
                ...
                var result = await _userCollection.DeleteOneAsync(x => x.Id == request.Id);
                if (!result.IsAcknowledged)
                {
                    response.IsSuccess = true;
        ...
        public async Task<DeleteAllRecordResponse> DeleteAllRecord()
        {
            ...
            try
            {
                var Result = await _userCollection.DeleteManyAsync(x => true);
                if (!Result.IsAcknowledged)
                {
                    response.IsSuccess = false;
